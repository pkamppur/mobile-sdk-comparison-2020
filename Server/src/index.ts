...
app.get('/api/games/:id/details', async (req, res, next) => {
    try {
        ...
        const playersCommunityBest: { numPlayers: number, votes: number } = game.poll.filter(poll => poll.$.name === 'suggested_numplayers')
            ?.[0].results.map(x => {
                return {
            ...
            .sort((one, two) => (one.votes > two.votes ? -1 : 1))
            ?.[0].numPlayers
        ...
        const result = {
            ...
            yearPublished: game.yearpublished?.[0]?.$?.value || '',
            description: htmlEntities.decode(game.description[0]),
...
function extractLinks(links: any[], type: string) {
    return links.filter(link => link.$.type === type).map(link => { return { name: link.$.value, id: link.$.id } })
}
